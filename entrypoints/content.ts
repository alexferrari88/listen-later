...
function extractContent() {
	try {
		...
		if (!window.currentJobId) {
			...
			throw new Error(
				"No job ID provided - content script not properly initialized",
			);
		}
					...
					if (originalText && originalText.length > 20) {
						...
						if (!seenTexts.has(originalText)) {
							...
							if (element.tagName.toLowerCase() === "li") {
								...
								if (parentOl) {
									...
									if (position !== -1) {
										...
										logger.debug(
											`Added numbering to list item: ${listNumber}. ${originalText.substring(0, 50)}...`,
										);
									}
								}
							}
							...
							if (!hasNestedContent || element.tagName.toLowerCase() === "li") {
								...
								logger.debug(
									`Added ${element.tagName}: ${finalText.substring(0, 50)}...`,
								);
							} else {
								// Still mark as seen to prevent duplicates, even if we don't include it
								seenTexts.add(originalText);
								logger.debug(
									`Skipped nested ${element.tagName}: ${originalText.substring(0, 50)}...`,
								);
							}
						} else {
							logger.debug(
								`Skipped duplicate content: ${originalText.substring(0, 50)}...`,
							);
						}
					}
				});
