	...
	test("extension extraction should have no duplicates", async ({ page }) => {
		...
		const extensionResult = await page.evaluate(() => {
			...
			const cleanText = extractedText
				...
				.replace(
					/\b(previous|next|page \d+( of \d+)?|more\.\.\.)\b[^\n.]*/gi,
					"",
				)
		...
		const lines = extensionResult.extractedText
			.split("\n\n")
		...
		console.log(
			`Total elements found: ${extensionResult.debugInfo.totalElements}`,
		);
		console.log(
			`Elements processed: ${extensionResult.debugInfo.processedElements}`,
		);
		console.log(
			`Duplicates skipped: ${extensionResult.debugInfo.skippedDuplicates}`,
		);
		console.log(
			`Nested elements skipped: ${extensionResult.debugInfo.skippedNested}`,
		);
