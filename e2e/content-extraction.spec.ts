...
test.describe("Content Extraction Quality", () => {
	...
	async function extractReferenceContent(page: any): Promise<{
		...
		if (article.content) {
			...
			const elements = contentDoc.querySelectorAll(
	...
	test("should extract high-quality content from Balaji blog post", async ({
		page,
	}) => {
		...
		const originalParagraphs = referenceContent.textContent
			.split(/\n+/)
			.filter((p) => p.trim().length > 0);
		const cleanedParagraphs = cleanedReference
			.split("\n\n")
			.filter((p) => p.trim().length > 0);
		console.log("Paragraph analysis:", {
			...
			originalLineBreakPattern:
				referenceContent.textContent.match(/\n+/g)?.slice(0, 5) || "none",
			sampleOriginalParagraphs: originalParagraphs
				.slice(0, 3)
	...
	test("should preserve article structure and key points", async ({ page }) => {
		...
		const paragraphs = cleanedReference
			.split("\n\n")
			.filter((p) => p.trim().length > 0);
		...
		const substantialParagraphs = paragraphs.filter(
			(p) => p.trim().split(/\s+/).length > 10,
		);
		expect(substantialParagraphs.length / paragraphs.length).toBeGreaterThan(
			0.7,
		);
	});
	...
	test("should remove UI noise and navigation elements", async ({ page }) => {
		...
		const lines = cleanedReference
			.split("\n")
			.filter((line) => line.trim().length > 0);
		const veryShortLines = lines.filter(
			(line) => line.trim().split(/\s+/).length < 4,
		);
		expect(veryShortLines.length / lines.length).toBeLessThan(0.1); // Less than 10% should be very short
	});

	test("extension extraction should match improved algorithm", async ({
		page,
	}) => {
		...
		const referenceParagraphs = cleanedReference
			.split("\n\n")
			.filter((p) => p.trim().length > 0);
		...
		console.log("Extension algorithm validation:", {
			...
			hasAllKeyPoints: keyPoints.every((kp) =>
				cleanedReference.toLowerCase().includes(kp.toLowerCase()),
			),
		});
	});
});
